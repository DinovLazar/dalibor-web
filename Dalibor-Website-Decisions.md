...
29. **Registered the revalidate webhook as a Sanity "document" (legacy) webhook via the management API, and hardened `/api/revalidate` to accept `slug` as a string OR a `{current}` object — rather than requiring the dashboard-only GROQ webhook.** — *Why:* the management API + `sanity hooks` CLI available on the build machine can only create legacy document webhooks (they ship the whole document, so `slug` is `{current}`, not the GROQ-projected string). Given the option to finish it here (Lazar's call) vs. hand off a dashboard GROQ webhook, the route was made robust to both payload shapes (good untrusted-input practice anyway) and the hook registered programmatically. Draft exclusion is preserved via `includeDrafts: false` (the brief's `!(_id in path("drafts.**"))` intent) plus the route's own `drafts.*` guard; the legacy hook can't carry a GROQ type filter, so non-`post`/`review` publishes also fire and are no-oped by the route. *(Surfaced in passing: the pre-existing reindex hook is the same legacy shape, so `/api/reviews/reindex` — which still requires a string slug — likely rejects its deliveries; out of scope, moot until the embeddings backfill.)*
