...
| `package.json` (2.01b) | + `import:content` script → `scripts/import-content.mts` (same `node --conditions=react-server --import tsx --env-file=.env.local` pattern). **(2.01e)** + `import:assets` → `scripts/import-assets.mts` (same runner) + `make:favicon` → `scripts/make-favicon.mts` (`node --import tsx`); **`sharp@0.34.5` pinned in devDeps** (favicon generation only — not shipped). **(2.12)** + `import:translations` → `scripts/import-translations.mts` (same runner). |
| `content-packet/` | `intake/Dalibor-Intake-Answers-MK.md` (verbatim MK bio §1 + book description §2 source) + `README.md`. **(2.01c)** + `topics.json` (13), `reviews.json` (20), `posts.json` (1) — the launch packet (copyright-safe: no body/excerpt). Read by `import-content.mts` via `readFileSync`+`JSON.parse` (no xlsx parser). `topics.json` is the canonical intended taxonomy + drives the importer's map-sync guard; the actual topic upsert **reconciles** onto Dalibor's live `t-*` taxonomy (see README). **(2.01e — now tracked)** + `assets/` — Cowork's image packet: `assets.json` manifest + `README.md` + `author/portrait.jpg` (720² interim author portrait) + `author/avatar-square.jpg` (512² favicon source crop). Read by `import-assets.mts`. **(2.01g)** + `reviewed-books/` — 19 of 20 reviewed-book covers (genuine flat portrait covers, web-sourced); `assets.json` `reviewed_books[]` extended to 20 entries (19 `use:true` + #11 held, self-published title with no findable cover). **(2.01h)** + `book/cover.jpg` — Dalibor's real Буники cover (operator-supplied, trimmed to 632×965), `book_cover.use:true`; 3 of the 19 `reviewed-books/` files (glasnik, esej-o-noci, svijet-koji-sam-izabrala — the current Home "latest 3 reviews") **replaced** with operator-supplied copies (lower-res than the superseded 2.01g web originals; esej-o-noci now sits under the 350px floor — flagged for Lazar). Live-Sanity check found **19/20 review `coverImage`s were already imported** (by someone with the write token, off-branch) before this phase started — only #11 + `book.coverImage` remained unset going in. **(2.01i)** + `reviewed-books/u-haosu-radosti-poetski-inat.jpg` — the last held reviewed-book cover, „Во хаосот радост" (Kalija Dimitrova), operator-supplied (1668×2000); `assets.json` `#11` flipped `use:"hold"` → `true` — **all 20/20 reviewed-book covers now `use:true`, 0 held.** |
...
| `src/app/[locale]/page.tsx` | **Real Style A Home (1.07)** — fetches the 4 `HOME_*` queries and composes Hero / Featured book / Latest reviews / From the blog (server, localized). **(1.12)** `generateMetadata` (absolute title) + `Person` JSON-LD. **(2.14)** the posts + reviews grids fetch via `clientFresh` + `force-cache` + `POST_TAG`/`REVIEW_TAG` so a publish refreshes them. |
...
| `src/app/[locale]/reviews/page.tsx` | **Real Style A Reviews list (1.09)** — `REVIEWS_LIST_QUERY` + `TOPICS_QUERY`; Archive header, SSR `?topic=` chip filter, progressively-enhanced `ReviewSearch` over the SSR `ReviewsList`. **(1.12)** `generateMetadata`. Dynamic (`ƒ`). **(2.14)** the list fetches via `clientFresh` + `force-cache` + `REVIEW_TAG` for webhook revalidation. |
| `src/app/[locale]/reviews/[slug]/page.tsx` | **Real Style A single review (1.09)** — `REVIEW_BY_SLUG_QUERY` (`cache()`d, shared w/ metadata); back link/breadcrumb, `<h1>`+double rule, Portable Text body w/ drop cap, sticky `ReviewBookAside`, topic chips; `generateStaticParams`, `notFound()`. **(1.12)** full `generateMetadata` (og:type article) + `Article`+`BreadcrumbList` JSON-LD + `lang` on title/body. **(2.09)** when `body` is empty (every review until Dalibor pastes prose), the left column renders a quiet "read at source" panel from the review's `source` instead of blank space. **(2.10)** the column is now a 3-branch render — `body` → **`summary`** (the `excerpt` field as Style A prose paragraphs w/ the "read at source" note beneath, fallback-language `lang`) → the 2.09 empty-state. **(2.14)** `getReview` fetches via `clientFresh` + `force-cache` + `reviewSlugTag(slug)` so a publish/delete refreshes (or 404s) this page. |
| `src/app/[locale]/blog/page.tsx` | **Real Style A Blog list (1.10)** — `POSTS_LIST_QUERY` + `TOPICS_QUERY`; Archive header, SSR `?topic=` chip filter (no dead chips), `PostCard` stack, empty state. **No search box.** **(1.12)** `generateMetadata`. Dynamic (`ƒ`). **(2.14)** the list fetches via `clientFresh` + `force-cache` + `POST_TAG` for webhook revalidation. |
| `src/app/[locale]/blog/[slug]/page.tsx` | **Real Style A single post (1.10)** — `POST_BY_SLUG_QUERY` (`cache()`d, shared w/ metadata); back link/breadcrumb, `<h1>`+double rule, Portable Text body w/ drop cap, topic chips; no book aside; `generateStaticParams`, `notFound()`. **(1.12)** full `generateMetadata` (og:type article) + `BlogPosting`+`BreadcrumbList` JSON-LD + `lang` on title/body. **(2.14)** `getPost` fetches via `clientFresh` + `force-cache` + `postSlugTag(slug)` so a publish/delete refreshes (or 404s) this page. |
...
| `src/app/api/reviews/reindex/route.ts` | **(1.09→2.03 LIVE)** `POST` re-index route — `x-webhook-secret`-authenticated (wrong/missing → 401; unconfigured → 503); builds the combined-language text via the shared `review-embedding-text` builder, embeds it, upserts the review's vector row by slug. Proven locally; live Sanity webhook registered by Cowork at the prod URL. **(2.14: byte-unchanged.)** |
| `src/app/api/revalidate/route.ts` | **(2.14)** `POST` publish→refresh route — same `x-webhook-secret` gate as reindex (503 unconfigured / 401 wrong-missing / 400 bad JSON / `drafts.*` ignored). Reads `{_type,_id,slug}` and `revalidateTag(tag,{expire:0})`s the tags behind the affected surfaces (`post`/`review` for index+Home, `post:<slug>`/`review:<slug>` for the single page). `readSlug()` accepts `slug` as a **string** (GROQ projection) OR a `{current}` **object** (legacy full-document webhook). Registered as a live Sanity webhook (id `9Xj3Y1ac51WdKzR0`, `includeDrafts:false`). |
...
| `src/app/globals.css` | Global stylesheet — Tailwind v4 + `tw-animate-css` imports + Style A `@theme` tokens **+ shadcn semantic alias tokens (1.06)** + the `.reveal` page-load animation (1.07) **+ (1.12) `scroll-margin-top` on focusable targets/headings (SC 2.4.11 — clear of the sticky header)** **+ (2.15) registered `@property --wordmark-shine-x` + one-shot `@keyframes wordmark-shine` for the header wordmark glint (both nuked by the reduced-motion block)**. **+ (3.01) the mobile foundation:** `--page-gutter` (one source of truth for the §4.6 gutter, 16px below 360px), the `full-bleed` utility (breaks out by negating the gutter, not `100vw`, so a scrollbar can never cause overflow), `.band-cream`/`.band-parchment` + the `--band-card` variable every card reads (so a card never sits on a ground of its own value; inert at ≥`sm`), `.card-surface`/`.card-flat`, the phone type scale as `@theme` token overrides under `width < 40rem`, `.tap-target` + a blanket `pointer: coarse` 44×44 hit area across header/footer/nav (via `::before`; the nav underline owns `::after`), the `.chip-rail` topic rail, `.list-cap`, `.to-top`, the italic-family remap for the split (unpreloaded) italic fonts, and the `page-gutter`/`safe-top`/`safe-bottom` `env(safe-area-inset-*)` utilities. |
| `src/app/manifest.ts` | **(3.01)** Web app manifest → `/manifest.webmanifest`. Deliberately minimal: this is a reading site, so `display: "browser"` keeps a saved-to-home-screen visit in the normal browser with its back button and share sheet. Colours are the locked cream page ground; icons reuse the existing `icon.png` / `apple-icon.png` metadata routes (no new assets). |
...
| `src/sanity/lib/client.ts` | Public read client (published perspective, `useCdn: true`, no token). **(2.14)** also exports **`clientFresh`** (`= client.withConfig({useCdn: false})`) — same published read but CDN-bypassing, used by the revalidatable blog/review/Home fetches so a busted cache tag re-fetches live truth (deterministic refresh). |
| `src/sanity/lib/tags.ts` | **(2.14)** Cache-tag names + per-slug builders (`POST_TAG`/`REVIEW_TAG`, `postSlugTag`/`reviewSlugTag`) — the single source shared by the pages that tag their fetches and `/api/revalidate` that busts them. |
...
| `scripts/import-translations.mts` | **(2.12)** Surgical patch importer — fills the **`en`/`sr` slots** of `author.roles`/`tagline`/`education`/`bio` + `book.description` (machine-drafted first draft, operator-authorized — see Decisions #26) so About/Book/Home read in-language instead of falling back to Macedonian. Uses `.patch().set()` on those keys **only** (never `createOrReplace` — that would wipe `author.photo`/`book.coverImage`); token-less read client does the diff so `--dry-run` needs no token; sets only fields that differ (idempotent); Zaporožac + Cyrillic-in-en/sr guards; re-reads to verify all 10 slots. `mk` never touched. `npm run import:translations`. |
...
| `scripts/mobile-audit.mts` | **(3.01)** The mobile harness. Drives headless Chrome over the DevTools Protocol using only Node built-ins (global `WebSocket`) — **no Playwright/Puppeteer**, because the phase forbids new dependencies. Walks 9 routes × 3 locales × 6 mobile + 3 desktop viewports and records document height, horizontal-overflow offenders, and every interactive element whose *effective* hit area is under 44×44 (probed by walking outward from the centre with `elementFromPoint`, so a pseudo-element hit area counts and one stolen by an overlapping neighbour correctly still fails). Classifies WCAG 2.2 SC 2.5.8 inline-prose exemptions separately. `--from`/`--to`/`--append` slice the run; a fresh browser per batch (headless Chrome destabilises after a few dozen tall captures). `npm run audit:mobile`. |
| `scripts/mobile-audit-report.mts` | **(3.01)** Builds `docs/mobile-audit/README.md` from the two measurement sets plus the Lighthouse summary. Separate from the harness so the report regenerates without re-driving a browser. `npm run audit:report`. |
| `scripts/lighthouse-mobile.mts` | **(3.01)** Runs Lighthouse in its **mobile** configuration over the six DoD pages and writes scores + LCP/CLS/TBT to `docs/mobile-audit/lighthouse.json`. Invoked through `npx` — an audit tool is not an app dependency. `npm run audit:lighthouse`. |
...
| `src/components/ui/shiny-button.tsx` | **New (2.15).** `'use client'`. Adapted from the 21st.dev ShinyButton to Style A: exports **`ShineOverlay`** (the reusable `aria-hidden`/`pointer-events-none`/`bg-clip-text` caramel→`#EAD1A0`→caramel foil glint, swept by the CSS `@keyframes wordmark-shine` via `group-hover:`/`group-focus-visible:`) — used by the header wordmark — and a `ShinyButton` primitive wrapping it (not mounted anywhere yet). `useReducedMotion()` gates the overlay out under reduced motion. No `dark:` variants, no infinite loop; sweep is CSS, not framer (framer would not tween the value here — see 2.15 report). |
...
| `src/components/layout/site-header.tsx` | **Style A sticky header** (Server Component) — wordmark→home (now rendered by the `Wordmark` client island, 2.15, with `t("wordmark")` passed in), desktop nav, switcher, mobile menu. **Also** the muted `xl`-only "builder credit" beside the wordmark: `common.builtBy` lead-in (**MK "Изработиле"**, 2.13) + a single caramel "Vertex Consulting" link (`siteLinks.vertex`); the credit `<p>` uses **`shrink-0 whitespace-nowrap`** (2.13) so it sits on **one line** (below `xl` it's hidden here and shown in the mobile menu instead). |
| `src/components/layout/wordmark.tsx` | **New (2.15).** `'use client'`. The header wordmark: a single locale-aware `<a href="/">` ("Dalibor Plečić", text passed in from the server header) with its original typography + focus-ring classes verbatim (only `group relative` added), carrying a decorative `ShineOverlay` glint on hover / keyboard focus. `useReducedMotion()` drops the overlay under reduced motion (clean static link). Accessible name + click target unchanged. |
...
| `src/components/layout/back-to-top.tsx` | **(3.01)** Client island for the two long article routes — a 48px thumb-zone control that appears after two screens, clear of the home indicator, phone-only. Toggles a `data-visible` attribute (never React state) from one rAF-coalesced scroll handler, so scrolling causes no re-render; `.to-top` transitions opacity/visibility only, so there is no motion for `prefers-reduced-motion` to disable. Moves focus to `<main id="content">` after the jump. |
...
| `src/components/topic-rail.tsx` | **(3.01)** Client wrapper around the topic chips — the only reason it is client code is to scroll the chip carrying `aria-current` into view on arrival (`inline: center`, `block: nearest`, no animation) when the row is a horizontal rail below `sm`. The chips themselves stay server-rendered, so the filter still works with JavaScript off. |
...
| `Part-1-Phase-02-Completion.md` … `Part-1-Phase-12-Completion.md`, **`Part-2-Phase-01b/01c/01d/01e/01g/01h/01i/02/03/04/05/09/10-Completion.md`** | Per-phase completion reports. |
...
| `docs/dalibor-publishing-guide.md` | **(2.04)** Non-technical publishing how-to for the live Studio (login → 3-language model → create post/review → cover+book metadata → draft vs publish). EN draft; MK version placeholder at top. **(2.14)** adds a plain-language "appears live within a minute or two (incl. the Vertex portal)" note under §6. |
...
### Mobile audit evidence — `docs/mobile-audit/` (3.01)

| Path | Purpose |
|---|---|
| `docs/mobile-audit/README.md` | Generated before/after report: document height per page per locale at 320/375/430, sub-44 target counts, horizontal-overflow counts, desktop non-regression, and the Lighthouse mobile table. |
| `docs/mobile-audit/before/` | Baseline: `measurements.json` (189 entries), `summary.md`, and 63 curated full-page screenshots. Captured from a pristine worktree of `a20d021` (pre-phase `main`) with the *final* harness, so both sets come from the same instrument. |
| `docs/mobile-audit/after/` | The same, post-phase. |
| `docs/mobile-audit/lighthouse.json` | Raw Lighthouse mobile scores + metrics for the six DoD pages. |
