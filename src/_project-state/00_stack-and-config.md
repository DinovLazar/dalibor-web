...
## 2026-07-12 — Phase 2.14 (publish → live-site refresh: `/api/revalidate` + cache tags)

**New route `src/app/api/revalidate/route.ts` (`ƒ`).** A secret-gated Sanity-webhook receiver mirroring `/api/reviews/reindex`'s gate (503 unconfigured / 401 wrong-or-missing `x-webhook-secret` / 400 bad JSON / `drafts.*` ignored). On a `post`/`review` it calls **`revalidateTag(tag, {expire: 0})`** — the Next 16 documented webhook-immediate-expiration form; the bare single-arg `revalidateTag(tag)` is now a **type error** (signature is `revalidateTag(tag, profile: string | {expire?: number})`). It is the ONLY new route; the build route table is otherwise unchanged (Home + single pages still `●` SSG, blog/reviews indexes still `ƒ`).

**Second read client — `clientFresh` in `src/sanity/lib/client.ts`.** `clientFresh = client.withConfig({useCdn: false})`; same published-only read as `client` but CDN-bypassing. The six revalidatable content fetches (Home posts+reviews grids, blog index, blog single, reviews index, reviews single) now go through `clientFresh` with `{cache: "force-cache", next: {tags: [...]}}`. **`client` (`useCdn: true`) is unchanged**, so the search orchestrator + reindex route read exactly as before (their code is byte-identical). *Rationale (verified, not assumed):* against a real prod build, `useCdn: true` made revalidation non-deterministic — the post-`revalidateTag` re-fetch reads Sanity's API CDN, which lags per-query and cached a stale render (a deleted post lingered on the index ~60–118 s; a Home grid never refreshed). `useCdn: false` on these reads gave a deterministic 55–400 ms refresh. Cache tags live in the new `src/sanity/lib/tags.ts` (`POST_TAG`/`REVIEW_TAG` + `postSlugTag`/`reviewSlugTag`), the single source shared by the pages and the route.

**Env / secret.** No new env var: `SANITY_WEBHOOK_SECRET` is **reused** to guard both `/api/reviews/reindex` and `/api/revalidate` (Decision #27). `.env.example` comment updated to say so; the real value stays in gitignored `.env.local` + on Vercel. The Sanity webhook (`https://dalibor-web.vercel.app/api/revalidate`, filter `!(_id in path("drafts.**"))`, projection `{_type,_id,"slug":slug.current}`) is **registered by the operator** post-merge (the sibling reindex hook already exists at the same base URL).

**No new dependency, no schema change (no `typegen` diff), no deploy from this machine, no Vercel env change.** `npm audit` unchanged.

## 2026-07-13 — Phase 2.15 (header wordmark glint)

**framer-motion 12.40 will not tween a value on this component — micro-animations go through CSS instead.** The 2.15 brief specified framer to drive the wordmark's `--x` sweep (`whileHover`/`whileFocus` or an `animate`-prop toggle, per the 21st.dev source). It does not work here: on the header wordmark's `motion.span`, framer animated **nothing** — not a CSS custom property in a variant label, not a direct `animate={{ "--x": … }}`, not a numeric `MotionValue` fed via `useMotionTemplate`, and **not even a plain `animate={{ opacity }}`** — all four were verified live in-browser (handlers fired, `active` state flipped, effects ran, `animate()` promise never resolved, value never moved). Root cause not fully isolated (React 19 StrictMode double-invoke + the app's existing `LazyMotion strict` mobile-menu island are suspects), but the practical rule stands: **the full `motion` component's value animation is unreliable in this app.** The working framer usage in `mobile-menu.tsx` uses `LazyMotion` + the lightweight `m` component for a mount transition, not value tweening. **Pattern adopted for 2.15 (and recommended for future hover/focus micro-interactions):** drive the animation with a **one-shot CSS `@keyframes` + a registered `@property`** (here `--wordmark-shine-x` + `@keyframes wordmark-shine` in `globals.css`), triggered by CSS `:hover`/`:focus-visible` (Tailwind `group-hover:`/`group-focus-visible:` arbitrary `[animation:…]`). framer's **`useReducedMotion()`** is still used, but only as the accessibility **gate** (drops the overlay under reduced motion) — not as the animator. The global `@media (prefers-reduced-motion: reduce) { * { animation: none } }` block is the CSS backstop. `@property --wordmark-shine-x { syntax: "<percentage>"; inherits: false; initial-value: 100% }` is required so the keyframe can interpolate the custom property (a bare, unregistered CSS var is not animatable). No new dependency (framer-motion already present; only `useReducedMotion` imported).
