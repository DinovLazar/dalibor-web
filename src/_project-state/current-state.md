...
**Last updated:** 2026-06-07 — Phase 1.08 complete (real Style A **About** + **Book** pages; shared Portable Text renderer; one schema tweak + seed discrepancy-guard on genre)
...
- **Next → 1.09** — styled **Reviews** list + topic search (and the single-review page — first real consumer of the Portable Text renderer, with the §3.6 drop cap). The thin proof/stub routes for reviews/blog/contact/privacy remain until their phases.
...
- **Data layer** `src/sanity/lib/`: `client.ts` (public read — published perspective, `useCdn`, **no token**), `image.ts` (`@sanity/image-url`), `queries.ts` (**8** typed `defineQuery`: reviews/posts list + **`ABOUT_QUERY`** (author singleton) + **`BOOK_QUERY`** (book-page projection incl. a cross-document `"authorName"` for the credit; no `genre`) + 4 Home-scoped `HOME_*` — the old generic `AUTHOR_QUERY` was removed in 1.08), `localize.ts` (`localizedValue` mk→en→sr + `availableLanguages`). `env.ts` validates the public env vars.
...
- **Thin connect-to-site proof routes** (1.05, temporary): `[locale]/reviews`, `/blog` — fetch from Sanity, render localized titles (+ cover / no-image); MK-only review falls back to mk + shows "available in: MK" on `/en`/`/sr`. Now render **inside the new chrome**, unchanged otherwise. Replaced by styled pages in 1.09–1.10.
...
- **Content (1.08):** `components/portable-text.tsx` — shared Style A Portable Text renderer (`@portabletext/react`), **locale-agnostic** server component. Styles every `blockContent` block/list/mark (Playfair h2–h4, Lora paragraphs, §6.10 blockquote, §3.5 lists, §2.2 caramel links with focus ring + auto external-link new-tab/`rel`), applies the `max-w-prose` measure, returns `null` when empty. Reused by About + Book now and by single review/blog/book pages later.
